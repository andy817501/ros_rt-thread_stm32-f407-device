...
void MotorControl::initMotors() {
    ...
    this->pwm_dev1 = (struct rt_device_pwm *)rt_device_find(PWM_DEV_NAME1);
    if (pwm_dev1 == RT_NULL)
    {
        rt_kprintf("pwm sample run failed! can't find %s device!\n", PWM_DEV_NAME1);
        rt_kprintf("error!!!!");
    ...
    rt_pwm_set(pwm_dev1, fl_for, maxSpd, 0);
    rt_pwm_enable(pwm_dev1, fl_for);

    rt_pwm_set(pwm_dev1, fl_back, maxSpd, 0);
    rt_pwm_enable(pwm_dev1, fl_back);

    rt_pwm_set(pwm_dev1, fr_for, maxSpd, 0);
    rt_pwm_enable(pwm_dev1, fr_for);

    rt_pwm_set(pwm_dev1, fr_back, maxSpd, 0);
    rt_pwm_enable(pwm_dev1, fr_back);
}
...
void MotorControl::move_andy(float speed,int channel)
   {

   if(speed>=0) {
    
    rt_pwm_set(pwm_dev, channel, maxSpd, speed);

   }
    else {
    rt_pwm_set(pwm_dev1, channel, maxSpd, -speed);
